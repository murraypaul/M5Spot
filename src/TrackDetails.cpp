...
TrackDetails TrackDetails::PopulateFromCurrentlyPlaying( HTTP_response_t response )
{
    ...
    if (!error) 
    {
        ...
        track.ArtURL[0] = json["item"]["album"]["images"][0]["url"].as<String>();
        track.ArtURL[1] = json["item"]["album"]["images"][1]["url"].as<String>();
        track.ArtURL[2] = json["item"]["album"]["images"][2]["url"].as<String>();
